...
def move_crates(mvmt):
    ...
    # For part two, we move the whole range of creates at one time
    # Grab the last x items from the list and move it to the other list
    stacks[to_st] += stacks[from_st][len(stacks[from_st]) - num_crates:]
    # Then remove them from the source list
    stacks[from_st] = stacks[from_st][:-num_crates]
